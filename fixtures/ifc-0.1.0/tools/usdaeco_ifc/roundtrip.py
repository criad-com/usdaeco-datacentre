...
def frame_camera(stage, paths, size=(1280, 800)):
    ...
    for path in paths:
        ...
        if not body or body.ComputeVisibility() == 'invisible' or body.ComputePurpose() != 'render':
            raise ValueError('Edited body must be visible with purpose render')
...
def roundtrip(source, model, directory):
    ...
    return session, comparison, selected
